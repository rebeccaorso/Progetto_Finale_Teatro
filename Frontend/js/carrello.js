  ...
  .then((spettacoli) => {
    ...
    fetch(URL3)
      ...
      .then((repliche) => {
        ...
        let dataRep = repliche.find(
          (replica) => replica.data_replica == dataReplica
        );
        console.log(dataRep);
        let codiceReplica;
        if (dataRep) {
        ...
        const codiceTrovato = spettacoli.filter(
          (spettacoli) => spettacoli.cod_spettacolo == codiceSpettacolo
        );
  ...
  <div>
    ...
    `;
          ...
          function applicaSconto() {
            const codiceScontoInserito = inputCodiceSconto.value
              .trim()
              .toLowerCase();

            if (codiceScontoInserito === "code15") {
...
const defaultElement = document.querySelectorAll(
  `.${defaultClass}:not([class*=""])`
);

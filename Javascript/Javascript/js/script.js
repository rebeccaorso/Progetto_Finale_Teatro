 ...
 */
function creaSelect(listaSpettacoli){  //assegnazione date alla select
...
function salvaDati() { //salva dati in local storage per ricaricarli nel carrello

    let data = null;
    let biglietti = null;

    
if(paginaCorrenteURL.includes("teatroCarignano.html")){ //seleziona la pagina html da tenere in considerazione
    if(checkBox1.checked == true){ //seleziona lo spettacolo checked
...
window.open("carrello.html", "_blank");
}


btn.addEventListener("click", function() { //button se non sei loggato non puoi comprare
